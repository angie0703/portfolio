...
from models.quefts_rice_yield import QUEFTS
...
d = {#'pH': np.column_stack((tsim, np.random.uniform(4.5, 7.4, len(tsim))))
     'pH':np.array([tsim, np.full((tsim.size,),pHvalues)]).T
     #'pH': np.random.uniform(4.5, 7.4, len(tsim))
     #'pH':7
     }

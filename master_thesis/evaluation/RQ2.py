...
Research Question 2: “What is the optimum kg/ha of organic fertilizers given to 
increase phytoplankton populations to supply at least 50% of the total feed required for fish?”

Goal: Minimum N and P kg/ha of organic fertilizer for Mphy to reach r_phy >= 0.5?
...
Norgf = (1.23/100)*1000 #N content in chicken manure 
...
##fish model
#state variables
# TODO: define the fish stocking density
N_fish = 21.84 #[g m-3]
pond_volume = pond_area * 0.5
n_fish = (N_fish*pond_volume)/18.2

x0fish = {'Mfish': 18.2*n_fish, #[gDM] 
      'Mdig':1E-6*n_fish, #[gDM]
      'Muri':1E-6*n_fish, #[gDM]
      } 

ufish = {'Mfed': 18.2 * 0.03 * n_fish}
...
# Rice model
# TODO: define planting density and number of plants
...
for ti in it:
    ...
    f_N_prt = flows_fish['f_N_prt'] if fish_active == True else 0
    ...
    f_TAN = flows_fish['f_TAN'] if fish_active == True else 0
    S_NH4 =  y1['P']+f_TAN
    ...
    #retrieve fish ouputs and phyto outputs as PSB inputs
    f_P_prt = flows_fish['f_P_prt'] if fish_active == True else 0
    ...
    f_P_sol = flows_fish['f_P_sol'] if fish_active == True else 0
    S_P = f_P_sol + y4['P']
    ...
    dphy = {
         ...
         'DVS':np.array([tsim, rice.f['DVS']]).T if rice_active else 0, 
         'pd':np.array([tsim, np.full((tsim.size,), 0.501)]).T,
           'S_NH4': np.array([tsim, y1['P']]).T,  #if not fish_active else np.array([tsim, S_NH4]).T,
         'S_NO2': np.array([tsim, y2['P']]).T,
         'S_NO3': np.array([tsim, y3['P']]).T,
         'S_P': np.array([tsim, y4['P']]).T #if not fish_active else np.array([tsim, S_P]).T
          }
    ...
    drice = {'I0':np.array([tsim, I0]).T,
             ...
             'S_NH4': np.array([tsim, y1['P']]).T if ti > 20 else 0,
             'S_NO3': np.array([tsim, y3['P']]).T if ti > 20 else 0,
             'S_P': np.array([tsim, S_P]).T if ti > 20 else 0
         }

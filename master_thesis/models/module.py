...
class Module():
    ...
    def run(self,tspan,d=None,u=None):
        ...
        if len(self.y) == 0:
            ...
            # print('y_keys: ', self.y_keys)
            for k in self.y_keys:
                self.y[k] = np.full((len(self.t),),np.nan)       
        for k in self.y_keys:
            idxs = np.isin(self.t,y['t'])
            # print("Indices (idxs) size:", np.sum(idxs))
            # print("Data array size (y[k]):", len(y[k]))
            # print("Target array segment size (self.y[k][idxs]):", len(self.y[k][idxs]))
            self.y[k][idxs] = y[k]

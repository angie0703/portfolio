...
class PNut(Module):
   ...
   y : dictionary
    ...
    def diff(self,_t,_x0):
        ...
        Nrice = _x0[3]    #Nutrient that is available for rice uptake
        ...
        # - phytoplankton
        mu_phy = self.p['mu_phy'] #[h-1] maximum growth rate of phytoplankton
        mu_Up = self.p['mu_Up'] #[h-1] maximum nutrient uptake coefficient
        ...
        Kpp = self.p['Kpp'] # [J (m2 h)-1]half-saturation constant of phytoplankton production
        c_prd = self.p['c_prd'] #[h-1] phytoplankton natural mortality constant
        ...
        kNdecr = self.p['kNdecr'] #[h-1] decomposition rate (to replace decomposition rate of bacteria)
        kPdecr = self.p['kPdecr'] #[h-1] decomposition rate (to replace decomposition rate of bacteria)

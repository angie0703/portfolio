...
dfp = {
    ...
    "SNO3": np.array([tsim, SNO3*pond_area*0.6]).T,
    "SP": np.array([tsim, SP*pond_area*0.6]).T,
...
# plt.plot(t, f_Nfert, label = 'N rate from inorganic fertilizer')
# plt.plot(t, price['IgN'], label = 'N rate from indigenous soil')
# plt.plot(t, f4*pond_area*0.6/1000, label = 'fraction of N from phytoplankton residue')
plt.plot(t, yfp['f_TAN'], label = 'TAN in fish urine')
# plt.plot(t, yfp['f_N_sol'], label='N content in fish urine')
plt.plot(t, yfp['f_N_prt'], label='N content in faeces')
plt.xlabel(r'time [d]')
plt.ylabel(r'rate $[kg d^{-1}]$')
plt.legend()
plt.title('Nitrogen flow rate')
...
plt.xlabel(r'time [d]')
plt.ylabel(r'Concentration $[g d^{-1}]$')
# plt.title(r'Accumulative Phytoplankton Growth and Nutrient Availability')
plt.legend()

plt.figure(6)
plt.plot(t, Mphy, label = 'Mphy')
plt.plot(t, fp.f['f_fed'], label='f_fed')
plt.xlabel(r'time [d]')
plt.ylabel(r'rate $[g d^{-1}]$')
# plt.title(r'Accumulative Phytoplankton Growth and Nutrient Availability')
plt.legend()

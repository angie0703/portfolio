...
Class for Monod model, adapted from lotka-volterra model
    ...
    x0 : dictionary
        ...
        * S : substrate concentration utilized by bacteria [g m-3]
        * X : bacteria concentration [g m-3 d-1]
        * P : product concentration [g m-3 d-1]
...
class DB(Module):
    ...
    def diff(self,_t,_x0):
        ...
        d_pond = self.p['d_pond'] #[m] pond depth
        A_pond = self.p['A_pond'] #[m2] pond area
        ...
        #supporting equations
        a_f = 1 if _t == 0 else 0
        S = S + (Norgf/d_pond)*a_f + _f3*kN + _f_N_prt/(d_pond*A_pond)
...
class PSB(Module):
    ...
    def diff(self,_t,_x0):
        ...
        d_pond = self.p['d_pond']
        A_pond = self.p['A_pond']
        ...
        Porgf = self.u['Porgf']
        a_f = 1 if _t == 0 else 0
        S = S + (Porgf/d_pond)*a_f + _f3*kP + _f_P_prt/(d_pond*A_pond)
        ...
        if f_P_sol.any() > 0:
            dP_dt = (-dS_dt/MrPhytate)*6*MrH2PO4 + _f_P_sol
        else:
            dP_dt = (-dS_dt/MrPhytate)*6*MrH2PO4

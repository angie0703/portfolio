...
class Rice(Module):
   ...
   def __init__(self, tsim, dt, x0, p):
       ...
       self.f_keys = ('f_Ph', 'f_Nlv', 'f_cgr', 'f_res', 'Cr_tot', 'f_uptN', 
                      'f_dmv', 'HU', 'N_lv', 'DVS', 'LAI', 'f_spi', 'NumGrain')
...
class Rice_hourly(Module):
   ...
   def diff(self, _t, _x0):
       ...
       #-N dynamics
       #to simulate rice seedlings planted in the nursery
       if _t >= 21*24:
      ...
      #HU units [ C d/d], DVR = [1/Cd], DVS = [-]
       ...
       k = 0.4 if 0 <= DVS< 1 else 0.8 
       
       d_lv = 0 if 0 <=DVS < 1 else 0.015 if 1<= DVS < 2 else 0.009  

       # Convert dictionary to lists
       DVS_values = [0, 0.16, 0.33, 0.65, 0.79, 2.1, 2.5] #[-]
       SLA_values = [0.0045, 0.004, 0.0033, 0.0028, 0.0024, 0.0023,0.0023] #[???]
       ...
       f_N_plt_upt = np.maximum(0, np.minimum(MuptN, _Nrice*Rec)) #[g h-1]
       f_uptN = f_N_plt_upt/1000 #[kg h-1] convert the nutrient source to kg

       print('N uptake of rice: ', f_uptN, 'gram')

...
n_fish = [4000, 8000, 12000, 10000] #[no of fish] according to Pratiwi 2019, 10000 fish is just for comparing result with literature
...
dfp_ = {'DVS': []}
drice_ = {'SNO3': [], 'SP': []}
yfp_ = {'t': [0, ], 'Mfish': [0, ], 'Mdig':[0, ], 'Muri':[0, ], 'Mphy': [4.29e-5, ], 'NA':[0.1, ], 'N_deposit':[0.1, ], 'P_deposit':[0.1, ] }
...
for ti in it:
    ...
    # Controlled inputs
    ufish = {'Mfed': m_fish*0.03*n_fish[0]/pond_volume} #[g m-3]
...
# plt.figure(2)
# modified_f_N_upt = np.where(t > 80, 0, fp.f['f_N_upt'])
# modified_f_P_upt = np.where(t > 80, 0, fp.f['f_P_upt'])
# modified_f_TAN = np.where(t > 80, 0, fp.f['f_TAN'])
# modified_f_N_sol = np.where(t > 80, 0, fp.f['f_N_sol'])
# modified_f_P_sol = np.where(t > 80, 0, fp.f['f_P_sol'])
# modified_f_N_prt = np.where(t > 80, 0, fp.f['f_N_prt'])
# modified_f_P_prt = np.where(t > 80, 0, fp.f['f_P_prt'])

# plt.plot(t, modified_f_N_upt/pond_volume, label = '$\phi_{N,upt}$')
# plt.plot(t, modified_f_P_upt/pond_volume, label = '$\phi_{P,upt}$')
# plt.plot(t, -modified_f_TAN/pond_volume, label = '$\phi_{TAN}$')
# plt.plot(t, -modified_f_N_sol/pond_volume, label='$\phi_{N,sol}$')
# plt.plot(t, -modified_f_P_sol/pond_volume, label='$\phi_{P,sol}$')
# plt.plot(t, -modified_f_N_prt/pond_volume, label='$\phi_{N,prt}$')
# plt.plot(t, -modified_f_P_prt/pond_volume, label='$\phi_{P,prt}$')
# # plt.plot(t, -fp.f['f_prt']/pond_volume, label='$\phi_{prt}$')
# # plt.plot(t, fp.f['f_fed']/pond_volume, label ='$\phi_{fed}$')
# plt.xlabel(r'time [d]')
# plt.ylabel(r'rate $[g m^{-3} d^{-1}]$')
# plt.legend()
# plt.title('Flow rates in Fish Model')
...
plt.figure(7)
plt.plot(t, f1, label='phytoplankton growth ($\phi_{f1}$)')
plt.plot(t, -f2, label= 'natural mortality rate ($\phi_{f2}$)')
plt.plot(t, -f3, label = 'death by intraspecific competition rate ($\phi_{f3}$)')
plt.xlabel(r'time [d]')
plt.ylabel(r'Flow rate $[g m^{-3} d^{-1}]$')
plt.title(r'Flows in $M_{phy}$')
plt.legend()

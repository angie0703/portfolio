...
n_fish = [4000, 5000, 6000] #[no of fish] according to Mridha 2014
...
#Instantiate object:
rcf00 = RCF(tsim, dt, x0, p, n_fish[0], n_rice[0])
rcf10 = RCF(tsim, dt, x0, p, n_fish[1], n_rice[0])
rcf20 = RCF(tsim, dt, x0, p, n_fish[2], n_rice[0])
rcf01 = RCF(tsim, dt, x0, p, n_fish[0], n_rice[1])
rcf11 = RCF(tsim, dt, x0, p, n_fish[1], n_rice[1])
rcf21 = RCF(tsim, dt, x0, p, n_fish[2], n_rice[1])

#First cycle Rainy season
yrcf001 = rcf00.run_simulation(tspan, d1, u, pond_volume)
yrcf101 = rcf10.run_simulation(tspan, d1, u, pond_volume)
yrcf201 = rcf20.run_simulation(tspan, d1, u, pond_volume)
yrcf011 = rcf01.run_simulation(tspan, d1, u, pond_volume)
yrcf111 = rcf11.run_simulation(tspan, d1, u, pond_volume)
yrcf211 = rcf21.run_simulation(tspan, d1, u, pond_volume)

#Second cycle Dry season
yrcf002 = rcf00.run_simulation(tspan, d2, u, pond_volume)
yrcf102 = rcf10.run_simulation(tspan, d2, u, pond_volume)
yrcf202 = rcf20.run_simulation(tspan, d2, u, pond_volume)
yrcf012 = rcf01.run_simulation(tspan, d2, u, pond_volume)
yrcf112 = rcf11.run_simulation(tspan, d2, u, pond_volume)
yrcf212 = rcf21.run_simulation(tspan, d2, u, pond_volume)

#Retrieve simulation results

t_ = yrcf001['NOB']['t']
def Mfish_fr(func):
    '''Return the fish fresh weight in kg DM'''
    Mfish_fr = func['Fish']['Mfish']/pfish['k_DMR']*0.001
    return Mfish_fr

def Mgr(func):
    '''Return the grain weight in ton DM'''
    Mgr = func['Rice']['Mgr']*0.001
    return Mgr

def y_results(func, dict_name):
    '''Retrieve the simulation results'''
    y = func[dict_name]
    return y

# #fish
def plot_rice(y_rice, title):
    '''Return plot of rice'''
    fig, ax =plt.subplots()
    t = y_rice['t']
    Mlv = y_rice['Mlv']
    ...
    Mpa = y_rice['Mpa']
    Mgr = y_rice['Mgr']
    ax.plot(t, Mlv, label = 'Leaves')
    ax.plot(t, Mst, label = 'stems')
    ax.plot(t, Mrt, label = 'roots')
    ax.plot(t, Mpa, label = 'panicles')
    ax.plot(t, Mgr, label = 'grains')
    ax.set_xlabel('Days [d]')
    ax.set_ylabel(r'Growth rate (g DM d$^{-1}$)')
    ax.set_title(title)
    ax.legend()
    
    
def plot_rice_fish(y_rice, y_fish):
    '''Return plot of rice and fish weight'''
    fig, ax1 =plt.subplots()
    t = y_rice['t']
    Mgr = y_rice['Mgr']*0.0001
    Mfish_fr = (y_fish['Mfish']/0.31)*0.001
    ax1.plot(t, Mgr, label = 'grains')
    ax1.set_xlabel('Days [d]')
    ax1.set_ylabel(r'Growth rate (ton DM d$^{-1}$)')
    ax1.legend()
    ax2 = ax1.twinx()
    ax2.plot(t, Mfish_fr,'b-', label = 'fish')
    ax2.set_ylabel(r'Growth rate (kg DM d$^{-1}$)')
    

def riceflows(func, dict_name, flow_name):
    '''Return the dictionary of flows from the functions'''
    flows = func[dict_name][flow_name]    
    return flows

#
yfish001 = y_results(yrcf001, 'Fish') 
yrice001 = y_results(yrcf001, 'Rice')

plot_rice_fish(yrice001, yfish001)
plt.show()
#rice flows
flow_rice1 = yrcf001['riceflow']
f_Ph1 = flow_rice1['f_Ph']
f_cgr1 = flow_rice1['f_cgr']
f_dmv1 = flow_rice1['f_dmv']
f_gr1 = flow_rice1['f_gr']
f_res1 = flow_rice1['f_res']
f_Nlv1 = flow_rice1['f_Nlv']
f_pN1 = flow_rice1['f_pN']
DVS1 = yrcf001['Rice']['DVS']
#phytoplankton
Mphy = yrcf001['Phyto']['Mphy']
NA = yrcf001['Phyto']['NA']

#bacteria
S_N_prt1 = yrcf001['DB']['S']
S_P_prt1 = yrcf001['PSB']['S']
S_NH41 = yrcf001['DB']['P']
S_NO21 = yrcf001['AOB']['P']
S_NO31 = yrcf001['NOB']['P']
S_P1 = yrcf001['PSB']['P']
X_DB1 = yrcf001['DB']['X']
X_AOB1 = yrcf001['AOB']['X']
X_NOB1 = yrcf001['NOB']['X']
X_PSB1 = yrcf001['PSB']['X']
# Create a figure and a grid of subplots
fig, axs = plt.subplots(2, 2, figsize=(12, 10))
fig.suptitle('Bacterial Growth, Substrate Utilization, and Substrate Production Rate: CRF Method')

# Plot results
axs[0, 0].plot(t_, S_N_prt1, label='Organic Matter Substrate')
axs[0, 0].plot(t_, X_DB1, label='Decomposer Bacteria')
axs[0, 0].plot(t_, S_NH41, label='Ammonium')
axs[0, 0].legend()
axs[0, 0].set_xlabel(r'$time\ [d]$')
axs[0, 0].set_ylabel(r'Concentration (g m$^{-3}$ d$^{-1}$)')
axs[0, 0].set_title("Decomposition of Organic Matter")

axs[0, 1].plot(t_, S_NH41, label='Ammonium')
axs[0, 1].plot(t_, X_AOB1, label='AOB')
axs[0, 1].plot(t_, S_NO21, label='Nitrite')
axs[0, 1].legend()
axs[0, 1].set_xlabel(r'$time\ [d]$')
axs[0, 1].set_ylabel(r'Concentration (g m$^{-3}$ d$^{-1}$)')
axs[0, 1].set_title("Nitrite production rate")

axs[1, 0].plot(t_, S_NO21, label='Nitrite')
axs[1, 0].plot(t_, X_NOB1, label='NOB')
axs[1, 0].plot(t_, S_NO31, label='Nitrate')
axs[1, 0].legend()
axs[1, 0].set_xlabel(r'$time\ [d]$')
axs[1, 0].set_ylabel(r'Concentration (g m$^{-3}$ d$^{-1}$)')
axs[1, 0].set_title("Nitrate production rate")

axs[1, 1].plot(t_, S_P_prt1, label='Organic P')
axs[1, 1].plot(t_, X_PSB1, label='PSB')
axs[1, 1].plot(t_, S_P1, label='Phosphate')
axs[1, 1].legend()
...
plt.figure(2)
plt.plot(t_, f_Ph1, label='Photosynthesis rate')
plt.plot(t_, f_res1, label='Maintenance respiration rate')
plt.plot(t_, f_gr1, label='Growth Respiration rate')
plt.plot(t_, f_dmv1, label='Death of leaves rate')
plt.xlabel(r'time [d]')
...
plt.figure(4)
plt.plot(t_, f_Nlv1, label='N flow rate in leaves')
plt.plot(t_, f_pN1, label = 'N flow rate in rice plants')
plt.xlabel(r'time [d]')
...
# plt.plot(t_, Mgr, label = 'Rice Grain Weight')
# plt.xlabel(r'time [d]')
# plt.ylabel(r'Biomass accumulation (g d$^{-1}$)')
# plt.title(r'Yield biomass accumulation')
# plt.legend()
# plt.show()
...
plt.xlabel(r'time [d]')
plt.ylabel(r'Biomass accumulation (g d$^{-1}$)')
plt.title(r'Rice biomass accumulation')
plt.legend()
plt.show()

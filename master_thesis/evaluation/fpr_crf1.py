...
tsim1 = np.linspace(0.0, t, t+1) # [d]
tsim2 = np.linspace(0.0, t, t+1) # [d]
...
dfish1 = {
    "DO": np.array([tsim1, np.linspace(4.9, 2.47, 120+1)]).T,
...
dphy = {
     ...
     'DVS':np.array([tsim1, np.linspace(0, 2.5, 120+1)]).T, #to simulate the information flows from rice growth
...
# dphy = {
#      'I0' :  np.array([tsim, np.full((tsim.size,), I0)]).T, #[J m-2 d-1] Average solar irradiation (6 AM to 6 PM) or 3,749,760 Joules per square meter per day (J m−2
...
x0rice = {
    "Mrt": 0.005*n_rice[0],  # [kg DM ha-1 d-1] Dry biomass of root
    "Mst": 0.003*n_rice[0],  # [kg DM ha-1 d-1] Dry biomass of stems
    "Mlv": 0.002*n_rice[0],  # [kg DM ha-1 d-1] Dry biomass of leaves
...
for ti in it:
    ...
    #run rice model phase 1
    yrice = rice1.run(tspan, drice1, urice)
    ...
    dphy_['DVS'].append(np.array([yrice['t'][1], rice1.f['DVS'][1]]))
    ...
    if ti < 24:
       ...
       yfish_['f_P_sol'].append(0)
           ...
           #make the fish weight zero to simulate 'no growth'
           yfish0 =  fish0.run((80, 120), dfish1, ufish)
           yfish_['t'].append(yfish0['t'][1])
           yfish_['Mfish'].append(0)
           yfish_['Mdig'].append(0)
           yfish_['Muri'].append(0)
       ...
       yfishr =   fish1.run((24,79), dfish1, ufish)
       ...
       #retrieve fish result as disturbances for rice and phytoplankton
       drice1_['SNO3'].append(np.array([yfishr['t'][1], yfishr['f_TAN'][1]]))
       drice1['SNO3'] = np.array([yfishr['t'], yfishr['f_TAN']])
       drice1_['SP'].append(np.array([yfishr['t'][1], yfishr['f_P_sol'][1]]))
       drice1['SP'] = np.array([yfishr['t'], yfishr['f_P_sol']])
       dphy_['SNO3'].append(np.array([yfishr['t'][1], yfishr['f_TAN'][1]]))
       dphy['SNO3'] = np.array([yfishr['t'], yfishr['f_TAN']])
       dphy_['SP'].append(np.array([yfishr['t'][1], yfishr['f_P_sol'][1]]))
       dphy['SP'] = np.array([yfishr['t'], yfishr['f_P_sol']])
       
      
        
    # #run rice model phase 2
    # yrice = rice2.run((121,240), drice2, urice)
    # yrice_['t'].append(yrice['t'][1])
    # yrice_['Mgr'].append(yrice['Mgr'][1])
    # yrice_['Mrt'].append(yrice['Mrt'][1])
    # yrice_['Mst'].append(yrice['Mst'][1])
    ...
    # yrice_['DVS'].append(yrice['DVS'][1])
    # #retrieve yrice results as phytoplankton disturbances
    # dphy_['DVS'].append(np.array([yrice['t'][1], rice2.f['DVS'][1]]))
    # dphy['DVS'] = np.array([yrice['t'], yrice['DVS']])
    
    # # Retrieve phyto model outputs 
    # dfish2_['Mphy'].append(np.array([yphy['t'][1], yphy['Mphy'][1]]))
    # dfish2['Mphy'] = np.array([yphy['t'], yphy['Mphy']*pond_volume])
    
    # #run fish model
    # if ti < 142:
    #    #make the fish weight zero to simulate 'no growth'
    #    yfish0 =  fish0.run((121,141), dfish2, ufish)
    #    yfish_['t'].append(yfish0['t'][1])
    #    yfish_['Mfish'].append(0)
    #    yfish_['Mdig'].append(0)
    #    yfish_['Muri'].append(0)
    ...
    # elif ti > 170:
    #        #make the fish weight zero to simulate 'no growth'
    #        yfish0 =  fish0.run((170, ti[-1]), dfish2, ufish)
    #        yfish_['t'].append(yfish0['t'][1])
    #        yfish_['Mfish'].append(0)
    #        yfish_['Mdig'].append(0)
    #        yfish_['Muri'].append(0)
    ...
    #        yfish_['f_P_sol'].append(0)
    # else: 
    #    yfishr =   fish2.run((142,197), dfish1, ufish)
    #    yfish_['t'].append(yfishr['t'][1])
    #    yfish_['Mfish'].append(yfishr['Mfish'][1])
    #    yfish_['Mdig'].append(yfishr['Mdig'][1])
    #    yfish_['Muri'].append(yfishr['Muri'][1])
...
for key in drice1.keys():
    if key in drice1_:
        replace_values(drice1[key], drice1_[key])    
...
Mpa = yrice['Mpa']
f_Ph1= rice1.f['f_Ph']
f_res1= rice1.f['f_res']
f_gr1 = rice1.f['f_gr']
f_dmv1 = rice1.f['f_dmv']
f_pN1 = rice1.f['f_pN']
f_Nlv1 = rice1.f['f_Nlv']
f_Ph2= rice2.f['f_Ph']
f_res2= rice2.f['f_res']
f_gr2 = rice2.f['f_gr']
f_dmv2 = rice2.f['f_dmv']
f_pN2 = rice2.f['f_pN']
f_Nlv2 = rice2.f['f_Nlv']
...
color = 'tab:blue'
ax2.plot(t, flow_fish1['f_fed'], label='Total Feed Intake Rate', color=color, linestyle='-', marker='o')
...
plt.figure(10)
plt.plot(t, f_Ph1, label='photosynthesis')
plt.plot(t, f_res1, label='maintenance respiration')
plt.plot(t, f_gr1, label='growth respiration')
plt.plot(t, f_dmv1, label='death leaf rate')
plt.xlabel(r'time [d]')
...
plt.figure(11)
plt.plot(t, f_Nlv1, label='N flow rate in leaves')
plt.plot(t, f_pN1, label = 'N flow rate in rice plants')
plt.xlabel(r'time [d]')

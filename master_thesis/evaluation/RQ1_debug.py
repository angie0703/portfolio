...
total = 120
tsim = np.linspace(0.0, total, int(total)+1) # [d]
dt = 1 #phyto, fish, rice
...
t_ini = '20221001'
t_end = '20230129'
t_weather = np.linspace(0, total, total+1)
...
dDB = {'DO': np.array([t_weather, np.random.randint(1,6, size=tsim.size)]).T, # [mg O2 L-1] random number between 1 - 5 to simulate dissolved oxygen in the pond 
...
##TODO: Phosphorus Solubilizing by PSB 
...
# TODO: Variables to store results from each model
y1 = {}
y2 = {}
y3 = {}
y4 = {}
...
for idx in range(len(tsim)-1):
    ...
    # Run DB model
    y1_results = db.run(tspan,dDB, uDB)
    y1['t'].append(y1_results['t'])
    y1['S'].append(y1_results['S'])
    y1['P'].append(y1_results['P'])
    y1['X'].append(y1_results['X'])
    print(f"DB Model Output at Index {idx}: {y1}")  # Print DB model output
    
    print('dAOB[S_NH4_DB]: ', dAOB['S_NH4_DB'])     
    # Run AOB model    
    y2_results = aob.run(tspan,dAOB)
    y2.append(y2_results)
    
    print(f"AOB Model Output at Index {idx}: {y2}")  # Print AOB model output
    
    # #run NOB model
    y3_results = nob.run(tspan,dNOB)
    y3.append(y3_results)
    print(f"NOB Model Output at Index {idx}: {y3}")  # Print NOB model output
    
    #run PSB model
    y4_results = psb.run(tspan,dPSB, uPSB)
    y4.append(y4_results)
    print(f"PSB Model Output at Index {idx}: {y4}")  # Print NOB model output
    
    # Retrieve DB model outputs for AOB model
    dAOB['S_NH4_DB'] = np.array([y1[0]['t'][-1], y1[0]['P'][-1]])
    # Retrieve AOB model outputs for NOB model
    dNOB['S_out']= np.array([y2[0]['t'][-1], y2[0]['P'][-1]])
    
    # #retrieve output of DB, AOB, and NOB as phytoplankton inputs (disturbances)
    # dphy['S_NH4'] = np.array([y1['t'][-1], y1['P'][-1]]).T
    # dphy['S_NO2'] = np.array([y2['t'][-1], y2['P'][-1]]).T
    # dphy['S_NO3'] = np.array([y3['t'][-1], y3['P'][-1]]).T
    # dphy['S_P'] = np.array([y4['t'][-1], y4['P'][-1]]).T

    # #Run phyto model 
    # yphy_results = phyto.run(tspan, dphy)
    # yphy.append(yphy_results)
    
    # #retrieve phyto ouput as DB and PSB inputs
    # dDB['f3'] = np.array([yphy['t'][-1], yphy['f3'][-1]]).T
    # dPSB['f3'] =np.array([yphy['t'][-1], yphy['f3'][-1]]).T
    
    # #retrieve Mphy as Fish inputs (disturbances)
    # dfish['Mphy']=np.array([yphy['t'][-1], yphy['Mphy'][-1]]).T
         
    # # Run fish model
    # yfish_results = fish.run(tspan, d=dfish, u=ufish)
    # yfish.append(yfish_results)
    
    # # Retrieve f_N_prt from fish and phytoplankton dead bodies as DB input
    # dDB['f_N_prt'] = np.array((yfish['t'][-1], yfish['f_N_prt'][-1])).T
    
    # # Retrieve fish outputs and phyto outputs as PSB inputs
    # dPSB['f_P_prt'] = np.array([yfish['t'][-1], yfish['f_P_prt'][-1]]).T
    
    # # Retrieve DB model outputs for AOB model
    # dAOB['f_TAN'] = np.array([yfish['t'][-1], yfish['f_TAN'][-1]]).T

    # #retrieve bacteria outputs as rice plant inputs
    # drice['S_NO3'] = np.array([y3['t'][-1], y3['P'][-1]]).T
    # drice['S_P'] = np.array([y4['t'][-1], y4['P'][-1]]).T
    # drice['f_P_sol'] = np.array([yfish['t'][-1], yfish['f_P_sol'][-1]]).T
    
    # #run rice model
    # yrice_results = rice.run(tspan, drice, urice)
    # yrice.append(yrice_results)
    
    # #retrieve rice output as phytoplankton inputs
    # dphy['DVS'] = np.array([yrice['t'][-1],yrice['DVS'][-1]]).T

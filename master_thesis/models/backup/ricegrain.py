...
class Rice(Module):
   ...
   def __init__(self, tsim, dt, x0, p):
       ...
       self.f = {}
       self.f_keys = ('f_Ph', 'f_Nlv', 'f_cgr', 'f_res', 'f_gr', 'f_dmv', 'f_pN', 'HU', 'DVS', 'N_lv', 'NumGrain', 'f_spi', 'ACDD')
   ...
   def diff(self, _t, _x0):
       ...
       _f_uptN = np.interp(_t,f_uptN[:,0],f_uptN[:,1])  # [ppm] Atmospheric CO2 concentration
       ...
       f_pN = min(M_upN, (_f_uptN + I_N*Rec_N*fac + IgN))
       ...
       #spikelet formation
       f_spi = f_Ph*gamma*spi #[number of spikelet formed d-1]
       
       #number of grain formation
       dNumGrain_dt = f_spi*SPF

...
p_pn = {
      ...
      'f_N_edg': 0.8*0.6*1000/24, # [g h-1] endogenous N supply from soil [originally kg N ha-1 d-1]
      'f_P_edg': 0.5*0.6*1000/24, #[g h-1] endogenous P supply from soil [originally kg N ha-1 d-1]
      'MuptN': 8*0.6*1000/24, #[g h-1] maximum daily N uptake by rice plants, originally in [kg ha-1 d-1], only 0.6 ha of land that is planted with rice
      'MuptP': 5*0.6*1000/24,#[g h-1] maximum daily P uptake by rice plants, originally in [kg ha-1 d-1], only 0.6 ha of land that is planted with rice
...
price = {
      #Manually calibrated developmental rate
       'DVRi': 0, #[°C h-1]
       'DVRJ': 0.0020/24, # hourly development rate during juvenile phase [°C h-1], originally in [°C d-1]
       'DVRI': 0.00195/24,# hourly development rate during photoperiod-sensitive phase [°C h-1], originally in [°C d-1]
       'DVRP': 0.00195/24,# hourly development rate during panicle development phase [°C h-1], originally in [°C d-1]     
       'DVRR': 0.0024/24, # hourly development rate during reproduction (generative) phase [°C h-1], originally in [°C d-1]
         "Tmax": 40,#[°C]
         "Tmin": 15,#[°C]
         "Topt": 33,#[°C]
         "k": 0.4,
...
for ti in it:
    ...
    if ti < 24*24:
       #make the fish weight zero to simulate 'no growth'
        yfish0 =  fish0.run((0,23*24), dfish)
        ...
        d_pn_['f_N_sol'].append(np.array([yfish0['t'][1], 0]))
        d_pn_['f_P_sol'].append(np.array([yfish0['t'][1],0]))
        d_pn_['f_N_prt'].append(np.array([yfish0['t'][1], 0]))
        d_pn_['f_P_prt'].append(np.array([yfish0['t'][1], 0]))
        ...
        d_pn['f_P_prt'] = np.array([yfish0['t'], np.zeros_like(yfish0['t'])]).T
           ...
           #make the fish weight zero to simulate 'no growth'
            yfish0 =  fish0.run((80*24, tsim[-1]), dfish)
           # yfish0 =  fish0.run(tspan, dfish)
            yfish_['t'].append(yfish0['t'][1])
            yfish_['Mfish'].append(0)
            yfish_['Mdig'].append(0)
       ...
       yfishr =   fish.run((24*24,79*24), dfish)

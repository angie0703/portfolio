...
class PNut(Module):
    ...
    def diff(self,_t,_x0):
        ...
        # -- Controlled inputs
        I_N = self.u['I_N'] #[kg] N concentration in inorganic fertilizer
        I_P = self.u['I_P'] #[kg] P concentration in inorganic fertilizer
        ...
        nutrient_avail = [0.8, 0.72, 0.45, 0.21] #[d-1]
        rainfall_category = [ 0, 20, 100, 150] #[mm]
        dr = np.interp(_Rain, rainfall_category, nutrient_avail) #[%] Rain-adjusted nutrient availability
        ...
        f_N_fert_ing = I_N if _t == 33 else 0
        f_P_fert_ing = I_P if _t == 33 else 0
        ...
        #uptake from rice plants, it starts at t = 21 days to simulate rice plants transplanted to the field
        Rec = 0.3 if 0 <= _DVS < 0.4 else 0.5 if 0.4 <= _DVS < 1 else 0.75 #[-] percemtage of the nutrient can be uptake by the plant depend of the developmental stage (DVS)
        f_N_plt_upt = np.minimum(MuptN, Rec*((f_N_edg + f_N_fert_ing + f_N_fert_org + _f_N_sol + _f_N_prt + f_N_phy_cmp)*dr - f_N_phy_upt)) if _t>=21 else 0 #[g d-1]
        f_P_plt_upt = np.minimum(MuptP, Rec*((f_P_edg + f_P_fert_ing + f_P_fert_org + _f_P_sol + _f_P_prt + f_P_phy_cmp)*dr - f_P_phy_upt)) if _t>=21 else 0#[g d-1]

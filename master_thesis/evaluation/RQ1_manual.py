...
##Decomposer Bacteria
x0DB = {
    "S": 0.323,  # Particulate Matter concentration
...
# Rice model
n_rice1 = 127980  # number of plants using 2:1 pattern in 0.6 ha land

x0rice = {
    "Mrt": 0.005*n_rice1,  # [kg DM ha-1 d-1] Dry biomass of root
    "Mst": 0.003*n_rice1,  # [kg DM ha-1 d-1] Dry biomass of stems
    "Mlv": 0.002*n_rice1,  # [kg DM ha-1 d-1] Dry biomass of leaves
...
dphy = {
    ...
    "SNH4": np.array([tsim, np.full((tsim.size,), 0.083)]).T,
    "SNO2": np.array([tsim, np.full((tsim.size,), 0.305)]).T,
    "SNO3": np.array([tsim, np.full((tsim.size,), 0.053)]).T,
    "SP": np.array([tsim, np.full((tsim.size,), 0.03278)]).T,
}
...
y1a = {key: np.array(value) for key, value in y1_results.items()}
y2a = {key: np.array(value) for key, value in y2_results.items()}
y3a = {key: np.array(value) for key, value in y3_results.items()}
y4a = {key: np.array(value) for key, value in y4_results.items()}
yphya = {key: np.array(value) for key, value in yphy_results.items()}
yfisha = {key: np.array(value) for key, value in yfish_results.items()}
yricea = {key: np.array(value) for key, value in yrice_results.items()}
...
# bacteria
S_N_prt = y1["S"]*pond_area*0.5
S_P_prt = y4["S"]*pond_area*0.5
S_NH4 = y1["P"]*pond_area*0.5
S_NO2 = y2["P"]*pond_area*0.5
S_NO3 = y3["P"]*pond_area*0.5
S_P = y4["P"]*pond_area*0.5
X_DB = y1["X"]*pond_area*0.5
X_AOB = y2["X"]*pond_area*0.5
X_NOB = y3["X"]*pond_area*0.5
X_PSB = y4["X"]*pond_area*0.5

# phytoplankton
Mphy = yphy["Mphy"]*pond_area
NA = yphy["NA"]*pond_area
...
axs[0, 0].set_ylabel(r'$growth rate\ [g d^{-1}]$')
...
axs[0, 1].set_ylabel(r'$growth rate\ [g d^{-1}]$')
...
axs[1, 0].set_ylabel(r'$growth rate\ [g d^{-1}]$')
...
axs[1, 1].set_ylabel(r'$growth rate\ [g d^{-1}]$')

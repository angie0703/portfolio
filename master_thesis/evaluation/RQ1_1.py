...
for ti in it:
    ...
    # Run DB model
    dDB['f3'] = np.array([yPhy['t'], yPhy['f3']])
    y1 = db.run(tspan,dDB, uDB)
    
    # Retrieve DB model outputs for AOB model
    dAOB['S_NH4_DB'] = np.array([y1['t'], y1['P']])
    ...
    #run PSB model
    dPSB['f3'] =np.array([yPhy['t'], yPhy['f3']])
    y4 = psb.run(tspan,dPSB, uPSB)
        
    #retrieve output of DB, AOB, and NOB as phytoplankton inputs (disturbances)
    dphy['S_NH4'] = np.array([y1['t'], y1['P']])
    dphy['S_NO2'] = np.array([y2['t'], y2['P']])
    dphy['S_NO3'] = np.array([y3['t'], y3['P']])
    dphy['S_P'] = np.array([y4['t'], y4['P']])
    

    #retrieve Mphy as Fish inputs (disturbances)
    dfish['Mphy']=np.array([yPhy['t'], yPhy['Mphy']])
         
    # Run fish model
    yfish = fish.run(tspan, d=dfish, u=ufish)
    
    # Retrieve f_N_prt from fish and phytoplankton dead bodies as DB input
    dDB['f_N_prt'] = np.array((yfish['t'], yfish['f_N_prt']))
    
    # Retrieve fish outputs and phyto outputs as PSB inputs
    dPSB['f_P_prt'] = np.array([yfish['t'], yfish['f_P_prt']])
    
    # Retrieve DB model outputs for AOB model
    dAOB['f_TAN'] = np.array([yfish['t'], yfish['f_TAN']])

    #retrieve bacteria outputs as rice plant inputs
    drice['S_NO3'] = np.array([y3['t'], y3['P']])
    drice['S_P'] = np.array([y4['t'], y4['P']])
    drice['f_P_sol'] = np.array([yfish['t'], yfish['f_P_sol']])
...
plt.plot(rice.t, Mgr, label='Grains')
plt.xlabel(r'time [d]')
...
plt.figure(3)
plt.plot(rice.t, f_Ph, label='f_ph')
plt.plot(rice.t, f_res, label='f_res')
plt.plot(rice.t, f_gr, label='f_gr')
plt.plot(rice.t, f_dmv, label='f_dmv')
plt.xlabel(r'time [d]')
...
plt.figure(4)
plt.plot(rice.t, f_Nlv, label='N flow rate in leaves')
plt.plot(rice.t, f_pN, label = 'N flow rate in rice plants')
plt.xlabel(r'time [d]')

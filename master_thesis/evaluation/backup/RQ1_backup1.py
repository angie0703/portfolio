...
for key in x0fish:
    if key == 'n1':
        ...
        fish = Fish(tsim, dt, x0fish['n1'], pfish)
    elif key == 'n2':
        fish = Fish(tsim, dt, x0fish, pfish)
        ufish = {'Mfed': 18.2 * 0.03 * n_fish[1]}
    else:
        ufish = {'Mfed': 18.2 * 0.03 * n_fish[2]}  
...
for ti in it:
    ...
    if fish_active==True:
        ...
        # Retrieve DB model outputs for AOB model
        f_TAN = fish.f['f_TAN'] if fish_active == True else np.zeros(tsim.size)
        S_NH4 =  y1['P']+f_TAN
        x0AOB['S'] = np.array([y1['t'], S_NH4])

...
import IterTools
 ...
 includet("plant.jl")
    ...
    leaching::Bool=false #true: N leaching happened
    ...
    harvest_date_wheat=49#from emergence to harvest date
    harvest_date_faba=40 #from emergence to harvest date
    ...
    rowdistance_wheat::Float64=0.1 #row distance for wheat
    plantdistance_wheat::Float64=0.025 #plant distance for wheat
    nplant_wheat::Int64=8 #number of plants in a row for wheat
    nrows_wheat::Int64=2#number of rows for wheat
    rowdistance_faba::Float64=0.2 #row distance for faba bean
    plantdistance_faba::Float64=0.2 #plant distance for faba bean
    nplant_faba::Int64=0#number of plants in a row for faba bean
    nrows_faba::Int64=0#number of rows for faba bean
    ...
    field_length::Float64=max(plantdistance_wheat*nplant_wheat,plantdistance_faba*nplant_faba)
    field_width::Float64=rowdistance_wheat*nrows_wheat+rowdistance_faba*nrows_faba+distance_between_crops
    x::Float64=10
    y::Float64=10
...
##photosynthesis leaf level
    ...
    function VirtualPlantLab.feed!(turtle::Turtle,ss::Soilcell, vars)
        ...
        if turtle.message== "ray tracer"
           ...
           HollowCube!(turtle,length = ss.length, width = ss.width, height = ss.height,colors=RGBA((ss.Ni)/1000, (ss.Ni)/1000, 1-(ss.Ni)/1000,0.1),materials=ss.material)
        end
    ...
    function render_field(field, soil,filename::String)
        ...
        scene = Mesh(vec(field),message="ray tracer")
        Soil=Mesh([soil])
    ...
    RLDtotal_wheat= DataFrame(layer=1:20)
    lrootlength= DataFrame(layer=1:20)
    arootlength= DataFrame(layer=1:20)
    interRLDtotal= DataFrame(layer=1:20)
    plant=DataFrame()
    Field=DataFrame()
    root_wheat=DataFrame()
    lroot_wheat=DataFrame()
    soil_data=DataFrame()
    Rootnumber_wheat = DataFrame(layer = repeat(1:20, 30))
    halfRootnumber_wheat=DataFrame(part=["1","0"])
    halflrootlength_wheat=DataFrame(part=["1","0"])

        for j in 1:rep
            
            orientations_wheat =[rand()*360.0 for i = plantdistance_wheat/2:plantdistance_wheat:nplant_wheat*plantdistance_wheat-plantdistance_wheat/2, j = rowdistance_wheat:rowdistance_wheat:rowdistance_wheat*nrows_wheat]
            orientations_faba =[rand()*360.0 for i = plantdistance_faba/2:plantdistance_faba:nplant_faba*plantdistance_faba-plantdistance_faba/2, j = rowdistance_faba:rowdistance_faba:rowdistance_faba*nrows_faba]
            origins_wheat =[Vec(i,j,0) for i = plantdistance_wheat/2:plantdistance_wheat:nplant_wheat*plantdistance_wheat-plantdistance_wheat/2, j = rowdistance_wheat/2:rowdistance_wheat:rowdistance_wheat*nrows_wheat-rowdistance_wheat/2];
            origins_faba =[Vec(i,j+rowdistance_wheat*nrows_wheat+distance_between_crops,0) for i = plantdistance_faba/2:plantdistance_faba:nplant_faba*plantdistance_faba-plantdistance_faba/2, j = rowdistance_faba/2:rowdistance_faba:rowdistance_faba*nrows_faba];
            fieldwheat= [Plant.create_plant(origins_wheat[i],i,Int(ceil(i/nplant_wheat)),orientations_wheat[i],Plant.wheat_param) for i in 1:Int(nplant_wheat*nrows_wheat)];
            fieldfaba= [Plant.create_plant(origins_faba[i],i,Int(ceil(i/nplant_faba)),orientations_faba[i],Plant.faba_param) for i in 1:Int(nplant_faba*nrows_faba)];
            if intercrop==false&&nplant_wheat>0
                field=fieldwheat
            elseif intercrop==false&&nplant_faba>0
                field=fieldfaba
            elseif intercrop==true
            field = vcat(fieldwheat, fieldfaba)
            else
                field=missing
            end
        ...
        #calc_interRLD!(Wheat.get_aroot(fieldwheat[1]), Wheat.get_lroot(fieldwheat[1]),plantdistance_wheat,rowdistance_wheat,field_length,field_width,nplant_wheat,nrows_wheat)
        plant=vcat(plant, write_plant_data(field=field, time=i+sowingdate, sim=j))
        Field=vcat(Field, write_field_data(field=field, time=i+sowingdate, sim=j))
    ...
    #lroot_wheat=vcat(lroot_wheat, write_lroot_data(field=field, time=i+sowingdate, sim=j))
        soil_data = vcat(soil_data,write_soil_data(all_soils, time=i+sowingdate, sim=j))      
       ...
       Growth.reset_biomass!(field)
        ...
        calc_RLD!(fieldwheat,plantdistance_wheat,rowdistance_wheat,field_length,field_width,nplant_wheat,nrows_wheat)   
    ...
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phfield.csv",Field)

    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phplant.csv",plant)
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phroot.csv",root_wheat)
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phRootnumber.csv",Rootnumber_wheat)
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phlroot.csv",lroot_wheat)
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phsoil.csv",soil_data)


    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phhalfRootlength.csv",halflrootlength_wheat)
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phhalfRootnumber.csv",halfRootnumber_wheat)
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phRLD.csv", RLDtotal_wheat)
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/pharootlength.csv", arootlength)
    # CSV.write("C:/Users/angel021/VSCODE/intercropmodel/phlrootlength.csv", lrootlength)

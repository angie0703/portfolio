...
#Decomposition and Nitrification Bacteria
...
y1_results = {
    'SNprt':[0.323404255,],#Particulate Matter concentration 
    'SNH4': [0.08372093,],
    'SNO2':[0.305571392,],
    'XDB':[0.2,], #decomposer bacteria concentration
    'XAOB':[0.07,],   #AOB concentration
...
for ti in it:
    ...
    dphy["S_NH4"] = np.array([y1["t"], y1["SNH4"]]).T
    dphy["S_NO2"] = np.array([y1["t"], y1["SNO2"]]).T
    dphy["S_NO3"] = np.array([y1["t"], y1["P_NO3"]]).T
    dphy["S_P"] = np.array([y2["t"], y2["P"]]).T
